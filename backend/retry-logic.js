...
async function processRetry(payment) {
  ...
  if (success) {
    ...
    await sendPaymentRecoveredEmail(payment.email, payment.product, payment.amount);
    ...
    // Obtener max_retries desde config
    const maxRetriesConfig = getConfigValue('max_retries');
    ...
    if (newRetries >= maxRetries) {
      ...
      await sendPaymentPermanentFailEmail(payment.email, payment.product, payment.amount);
      ...
      // Programar siguiente reintento usando config de DB
      const retryIntervalsConfig = getConfigValue('retry_intervals') || process.env.RETRY_INTERVALS || '60,300,900';
      ...
      await sendRetryFailedEmail(payment.email, payment.product, payment.amount, newRetries, payment.retry_link);
