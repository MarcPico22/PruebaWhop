...
// ⚡ A/B TESTING: 3 variantes de headlines
// Cambiar la variable HEADLINE_VARIANT para probar diferentes versiones
const HEADLINE_VARIANT = 'A'; // 'A', 'B' o 'C'

const HEADLINES = {
  A: {
    // Variante A: Urgencia + Beneficio directo
    main: "Recupera pagos fallidos sin mover un dedo",
    sub: "Miles de euros se pierden cada mes en Whop por pagos que fallan. Nosotros los recuperamos automáticamente mientras duermes.",
    badge: "💸 Estás perdiendo dinero ahora mismo"
  },
  B: {
    // Variante B: Dolor + Pérdida económica
    main: "Deja de perder €1,000+ cada mes en pagos fallidos",
    sub: "El 7% de tus ingresos desaparece por tarjetas expiradas y fondos insuficientes. Recupera ese dinero. Automáticamente.",
    badge: "⚠️ El 7% de tus ingresos se evapora"
  },
  C: {
    // Variante C: ROI + Números concretos
    main: "Recupera el 85% de tus pagos fallidos en 24 horas",
    sub: "Cada pago que falla es dinero que nunca cobrarás. A menos que uses Whop Recovery. Sistema automático con 85% de tasa de éxito.",
    badge: "📊 85% de recuperación automática"
  }
};

const CURRENT_HEADLINE = HEADLINES[HEADLINE_VARIANT];

// Microsección "¿Cuánto dinero estás perdiendo?"
function MoneyLossCalculator() {
  const [monthlyRevenue, setMonthlyRevenue] = useState(15000);
  
  const averageFailureRate = 0.07;
      ...
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-10 border-2 border-orange-200">
          ...
          <div className="mt-8 text-xs text-gray-400">
            Variant {HEADLINE_VARIANT} • A/B Test
          </div>
        </div>
      </div>
    </section>
        ...
        <div className="max-w-5xl mx-auto text-center">
          <div className="inline-block mb-4 sm:mb-6 px-3 sm:px-4 py-1.5 sm:py-2 bg-red-100 border border-red-300 rounded-full">
            <span className="text-xs sm:text-sm font-semibold text-red-700">
              {CURRENT_HEADLINE.badge}
            </span>
          </div>
          
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black text-gray-900 mb-4 sm:mb-6 leading-tight">
            {CURRENT_HEADLINE.main.split(' ').map((word, i, arr) => {
              // Hacer las últimas 3 palabras en gradiente
              if (i >= arr.length - 3) {
                return (
                  <span key={i} className="bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
                    {word}{' '}
                  </span>
                );
              }
      ...
      <section id="how-it-works" className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-12">
            {[
              {step: "1", title: "Conecta Stripe", desc: "Dos clics. Cero configuración. Listo en 30 segundos.", icon: "🔌"},
              {step: "2", title: "Detectamos fallos", desc: "Identificamos cada pago que no se completó. Automático.", icon: "🔍"},
              {step: "3", title: "Recuperamos dinero", desc: "Reintentos inteligentes. 85% de éxito. Tú solo cobras.", icon: "💰"}
            ].map((item, i) => (
